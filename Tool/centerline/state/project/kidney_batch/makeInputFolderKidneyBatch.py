...
class CMakeInputFolder :
    ...
    def process(self) -> bool :
        ...
        outfolder_root = os.path.join(self.m_zipPath, f"dataRoot_{post_fix}", huid)
        ...
        def make_maskzip(dataRootPath : str, huid : str) :
            huidPath = os.path.join(dataRootPath, huid)
            maskPath = os.path.join(huidPath, "02_SAVE", "01_MASK")
            zipName = os.path.join(dataRootPath, f"{huid}_mask_miop")
            shutil.make_archive(zipName, 'zip', maskPath)
        
        make_maskzip(os.path.dirname(self.m_outputFullPath), huid)
