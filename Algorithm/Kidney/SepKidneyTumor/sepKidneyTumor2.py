...
class CSepKidneyTumor(blockKidney.CKidneyBase) :
    ...
    def process(self) :
        ...
        if self.m_tumorPhase == "" :
            print(f"SepKidneyTumor.process() : TumorPhase is None. Return.")
            return
        listExoPath = []

        flagExcludeCystExoOrg = False  # Tumor와 서로 다른 phase에 Cyst_exo가 있는 경우, Cyst_exo_new를 생성하므로 기존의 Cyst_exo는 separate 과정에서 제외 시켜야함.
        ...
        for path in listPath :
            ...
            if "exo" in path :
                ...
                if "Cyst_exo_new" in path :
                    flagExcludeCystExoOrg = True
                
        if flagExcludeCystExoOrg == True : 
            listExoPath.remove("Cyst_exo.nii.gz")
